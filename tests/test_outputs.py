...
The shared TerraformDriver._read_outputs (inherited by the OpenStack/AWS
providers) must flatten that to {name: value} so the DB/UI see the same shape
mock mode produces — and must fail soft (log + {}) instead of a bare `except`.
"""
import json
import subprocess
...
def test_get_outputs_flattens_terraform_envelope(monkeypatch, tmp_path):
    ...
    outputs = OpenStackProvider()._read_outputs(tmp_path)
...
def test_get_outputs_returns_empty_on_tool_failure(monkeypatch, tmp_path):
    ...
    assert OpenStackProvider()._read_outputs(tmp_path) == {}
